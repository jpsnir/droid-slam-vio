...
class FactorGraph:
    ...
    def add_factors(self, ii, jj, remove=False):
        ...
        if self.corr_impl == "volume":
            ...
            # print(f"shape corr block : {type(self.corr)} - {self.corr}")
    ...
    def rm_keyframe(self, ix):
        ...
        # print(f"REMOVE CONDITION: {m.shape} - {m}")
        # print(
        #     f"KEYFRAME - {ix} - shape of nodes - {self.ii_inac.shape}, {self.jj_inac.shape}"
        # )
        # print(f"Value ii_inac {self.ii_inac}")
    ...
    def update(
        ...
        # motion features
        # print(f"shapes before update = {self.ii.shape}, {self.jj.shape}")
        with torch.cuda.amp.autocast(enabled=False):
            coords1, mask = self.video.reproject(self.ii, self.jj)
            motn = torch.cat([coords1 - self.coords0, self.target - coords1], dim=-1)
            motn = motn.permute(0, 1, 4, 2, 3).clamp(-64.0, 64.0)
            # print(f"computed motion shape: {motn.shape}")
        ...
        # print(f"shape self.net = {self.net.shape}")
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # print(f"------------------------------")
            # print(f"poses length = {self.video.poses.shape}")
            # print(f"target shape  - {target.shape}, weight shape - {weight.shape}")
            # print(f"nodes shapes : ii = {ii.shape} and jj = {jj.shape}")
            # print(f"t0 - {t0}, t1 - {t1}")

            # print(f"height = {ht}, width = {wd}")

            # The arrangment should be
            # (n_images, height of image, width of image, 2d image point)
            # By permuting we obtain the x and y coordinates of each image
            ...
            # To log these sets of variables will store
            # all the weights, adjacency list and target values.
            self.target_total = target
            self.weight_total = weight
            self.ii_total = ii
            self.jj_total = jj
            self.damping = damping
            print(f"t0 for total {max(1, self.ii_total.min().item() + 1)}")
            self.video.ba(
                target,
                weight,
                damping,
    ...
    def add_proximity_factors(
        ...
        # finds proximity factors between t0 to last image in video buffer
        # considering each one of them as nodes,
        # also takes in t1 to last image in video buffer.
        # The frame graph establishes connections as node i to node j for building

        t = self.video.counter.value
        print(
            f"Adding proximity factors - for frames in range ii - {t0, t} and jj - {t1, t}"
        )
        ...
        print(
            f"Computed distance for adding nodes in frame graph: shapes : ii -\
                  {ii.shape.numel()}, jj - {jj.shape.numel()}, dist(ii, jj) - {d.shape.numel()}"
        )
        # Assign distance to certain entries in distance variable as infinity.
        d[ii - rad < jj] = np.inf
        d[d > 100] = np.inf
        # print(f"DISTANCE shape in depth video:{d}")
