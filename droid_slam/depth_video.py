...
class DepthVideo:
    ...
    def ba(
        ...
        with self.get_lock():
            ...
            # print(f"-------------------------------")
            # for index, (ix, jx) in enumerate(zip(ii[:1], jj[:1])):
            #     print(
            #         f"LOCATION : {index, ix.item(), jx.item()} \n- \
            #             POSE _ I \n {self.tstamp[ix]} = {self.poses[ix]} \n, \
            #             POSE _ J\n {self.tstamp[jx]}: {self.poses[jx]}, \n \
            #             DISPARITY: {self.disps[ix].shape}, \n \
            #             WEIGHT: {ii[index]} - {weight[index].shape} \n\
            #             TIMESTAMP: i - {self.tstamp[ix]}, j - {self.tstamp[jx]}"
            #     )
